...
class EncryptedDataPackage:
    ...
    def from_json(cls, json_data: str):
        ...
        try:
            ...
            alice_pubkey_bytes = bytes.fromhex(data['alice_pubkey'])
            data_source_pubkey_bytes = bytes.fromhex(data['data_source_pubkey'])
            kit_bytes = bytes.fromhex(data['kit'])
            label_bytes = bytes.fromhex(data['label'])
            policy_pubkey_bytes = bytes.fromhex(data['policy_pubkey'])
    ...
    def decrypt(self, channel_reader: object) -> bytes:
        ...
        # retrieve(self, message_kit, data_source, alice_verifying_key, label):

        return channel_reader.BOB.retrieve(
            message_kit_object,
            enrico=data_source_restored,
